...
export default function BuildBrief({ brief, projectName, decision, onNew, onClose, saveStatus, onRetrySave }) {
  ...
  const handleShare = async () => {
    const file = new File([brief], 'BRIEF.md', { type: 'text/plain' });
  ...
  return (
    ...
    <div style={{ width: '100%', maxWidth: isDesktop ? 760 : undefined, height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', padding: '16px 16px 0', gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: '#E0E0E0' }}>Brief de construcción</div>
          {saveStatus === 'saving' && (
